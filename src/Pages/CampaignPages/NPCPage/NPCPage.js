...
import './NPCPage.css';
import { useDeleteNPCMutation, useGetNPCDataQuery, useUpdateNPCMutation } from '../../../features/campaign/campaignApiSlice';
...
function NPCPage() {
    ...
    const filteredNPCs = npcs.filter(character => character.character_id === characterId)
    ...
    const handleClick = event => {hiddenCharacterFileInput.current.click()}
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        await updateNPC({campaignId, characterId, formData: form_data}).unwrap();
        setDelCount(0);
          ...
          <>
            ...
            <Grid container spacing={2}>
              <Grid item xs={12} md={6}>
                ...
                <img  className="char-page-pc-img" src={(charData?.image) ? charData.image : notGiven} />
                <div className='char-page-desc-wrapper'>
                  <i>
                    {charData.description}
                  </i>
