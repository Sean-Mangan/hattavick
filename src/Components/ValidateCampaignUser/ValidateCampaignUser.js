...
const ValidateCampaignUser = () => {
    ...
    const {data: campaigns, isLoading: campaignsLoading, isSuccess: campaignsSuccess , isError} = useGetCampaignsQuery()
    var adminCampaigns = (campaignsSuccess) ? campaigns.admin.concat(campaigns.owner) : []
    var isAdmin = adminCampaigns.map((campaign) => campaign?.id).includes(campaignId)
    var isOwner = ((campaignsSuccess) ? campaigns.owner : []).map((campaign) => campaign?.id).includes(campaignId)


    // Get all them datas
    const {data: homepage, isLoading: homeLoading, isSuccess: homeSuccess , isError: homeError} = useGetCampaignHomeDataQuery(campaignId, {skip: !campaignsSuccess})
    const {data: party, isLoading: partyLoading, isSuccess: partySuccess , isError: partyError} = useGetPartyDataQuery(campaignId, {skip: !campaignsSuccess})
    const {data: npcs, isLoading: npcLoading, isSuccess: npcSuccess , isError: npcError} = useGetNPCDataQuery(campaignId, {skip: !campaignsSuccess})
    const {data: world, isLoading: worldLoading, isSuccess: worldSuccess , isError: worldError} = useGetWorldLoreQuery(campaignId, {skip: !campaignsSuccess})
    const {data: factions, isLoading: factionsLoading, isSuccess: factionsSuccess , isError: factionsError} = useGetFactionsQuery(campaignId, {skip: !campaignsSuccess})
    const {data: locations, isLoading: locationsLoading, isSuccess: locationsSuccess, isError: locationsError} = useGetLocationQuery(campaignId, {skip: !campaignsSuccess})
    ...
    useEffect(()=>{
        ...
        setAllCampaignIds(adminCampaigns.concat(ownerCampaigns, playerCampaigns).map((campaign) => campaign?.id))
    },[campaigns])

    // Make sure to set if the request is loading, or is errored
    useEffect(()=> {if (!hasLoaded && (loadingList.every((isLoading) => !isLoading))) setHasLoaded(true)}, loadingList)
        ...
        { (!hasLoaded) ?
            ...
            <>
                {(!allCampaignIds.includes(campaignId))
                    ? <div style={{textAlign:"center"}}><h1>You Do not Have Permission to View this Campaign.</h1></div>
                    : <Outlet context={{campaignId, isAdmin, isOwner}}/>
                }
            </>
        }
        </>
